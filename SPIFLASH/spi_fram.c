...
//* Begin    	
	FRAM_CS_LOW();       
	SPI_SendByte(SPI2, AT25_WRSR_CMD );
	SPI_SendByte(SPI2, status_reg_value );
...
//* Begin    
	...
	SPI_SendByte(SPI2, AT25_WRITE_CMD);
	
	/* Send ReadAddr high nibble address byte to read from */
	SPI_SendByte(SPI2, (address& 0xFF00) >> 8);
	/* Send ReadAddr low nibble address byte to read from */
	SPI_SendByte(SPI2, address & 0x00FF);

	/* Send "Write Enable" instruction */
	SPI_SendByte(SPI2, byte);
...
//* Begin    
	...
	SPI_SendByte(SPI2, AT25_READ_CMD);
...
void SPI_FRAM_WriteEnable(void)
{
	...
	/* Send "Write Enable" instruction */
	SPI_SendByte(SPI2, FM25CL64_WREN_INST);
...
void SPI_FRAM_WaitForWriteEnd(void)
{
	...
	SPI_SendByte(SPI2, FM25CL64_RDSR_INST);
...
{	
	...
	SPI_SendByte(SPI2, FM25CL64_WRITE_INST);

	/* Send ReadAddr high nibble address byte to read from */
	SPI_SendByte(SPI2, (WriteAddr& 0xFF00) >> 8);
	/* Send ReadAddr low nibble address byte to read from */
	SPI_SendByte(SPI2, WriteAddr & 0x00FF);

	/* Send "Write Enable" instruction */
	SPI_SendByte(SPI2, WriteData);
...
uint8_t SPI_FRAM_ByteRead(uint8_t ReadData, uint16_t ReadAddr)
{
	/* Select the FRAM: Chip Select low */
	FRAM_CS_LOW();
	SPI_SendByte(SPI2, FM25CL64_READ_INST);

	/* Send ReadAddr high nibble address byte to read from */
	SPI_SendByte(SPI2, (ReadAddr & 0xFF00) >> 8);
	/* Send ReadAddr low nibble address byte to read from */
	SPI_SendByte(SPI2,  ReadAddr & 0x00FF);


	ReadData = SPI_SendByte(SPI2, 0x00);
