...
extern unsigned char block_buff[SD_BLOCK_SIZE];//SD_BLOCK_SIZE
/**
  * @brief  Dummy byte
  */
...
extern u8 SD_SPI_SendCmd(u8 Cmd, u32 Arg, u8 Crc);
extern void SD_SendCmd(uint8_t Cmd, uint32_t Arg, uint8_t Crc);
...
void SD_DMA_Read(uint8_t* pBuffer);
void SD_DMA_Write(uint8_t* pBuffer);
SD_Error SD_ReadBlock_FS(uint8_t* pBuffer, uint32_t ReadAddr, uint16_t BlockSize);
