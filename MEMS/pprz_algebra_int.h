...
#define FLOAT_OF_BFP(_vbfp, _frac) ((double)(_vbfp)/(1<<(_frac)))
...
#define INT32_EULERS_OF_RMAT(_e, _rm) {					\
    									\
    const double dcm00 = TRIG_FLOAT_OF_BFP((_rm).m[0]);			\
    const double dcm01 = TRIG_FLOAT_OF_BFP((_rm).m[1]);			\
    const double dcm02 = TRIG_FLOAT_OF_BFP((_rm).m[2]);			\
    const double dcm12 = TRIG_FLOAT_OF_BFP((_rm).m[5]);			\
    const double dcm22 = TRIG_FLOAT_OF_BFP((_rm).m[8]);			\
    const double phi   = atan2f( dcm12, dcm22 );				\
    const double theta = -asinf( dcm02 );				\
    const double psi   = atan2f( dcm01, dcm00 );				\
...
#define INT32_EULERS_OF_QUAT(_e, _q) {					\
    ...
    const int32_t idcm22 = one - INT_MULT_RSHIFT( two, (qx2+qy2),	\
						  INT32_TRIG_FRAC+INT32_QUAT_FRAC-INT32_TRIG_FRAC); \
    const double dcm00 = (double)idcm00/(1<<INT32_TRIG_FRAC);		\
    const double dcm01 = (double)idcm01/(1<<INT32_TRIG_FRAC);		\
    const double dcm02 = (double)idcm02/(1<<INT32_TRIG_FRAC);		\
    const double dcm12 = (double)idcm12/(1<<INT32_TRIG_FRAC);		\
    const double dcm22 = (double)idcm22/(1<<INT32_TRIG_FRAC);		\
									\
    const double phi   = atan2f( dcm12, dcm22 );				\
    const double theta = -asinf( dcm02 );				\
    const double psi   = atan2f( dcm01, dcm00 );				\
